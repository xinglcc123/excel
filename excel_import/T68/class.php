...
class excel_import extends basic{
	...
	public function __construct()
	{
		...
		$this->error_files=[
			...
			"all_banklevel_no"					=> "import_log/".$this->prefix."_all_banklevel_no_跳過的會員組.log", //跳過的會員組20211110arvin
			"all_banklevel_sort"					=> "import_log/".$this->prefix."_all_banklevel_會員組" //分類用的會員組20211110arvin
		];
		if($this->isupdate == 0){
	...
	private function importmember()
	{
		...
		if(count($insert_data) > 0){
			foreach($insert_data as $row)
			{
				...
				//需要跳過的會員組20211110arvin
				if($row['banklevel'] == '不出款会员！ (12)'){
					echo(color('用户名: '. $row['account'] . ' 会员组 ' . $row['banklevel'] . ' 略過 '.PHP_EOL,'Light Red'));
					file_put_contents($this->error_files['all_banklevel_no'],$row['account']."\t".$row['name']."\t".$row['banklevel'].PHP_EOL,FILE_APPEND);
					continue;
				// } else {	//輸出會員組清單20211110arvin
				// 	file_put_contents($this->error_files['all_banklevel_sort'].'_'.$row['banklevel'].'.log',$row['account']."\t".$row['name']."\t".$row['banklevel'].PHP_EOL,FILE_APPEND);
				}
	...
	public function check_accountcode($str,$old,$times=1,$name){
		...
		if($times > 10){ //遞迴了十次直接掉過
			...
			file_put_contents($this->error_files['member_change'],$old."\t".$str.PHP_EOL,FILE_APPEND);	//匯入帳號
		}
		return $str;
	}
