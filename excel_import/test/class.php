<?php
// ini_set('date.timezone', 'America/New_York');	// 固定-4时区
// ini_set('date.timezone', 'Europe/London'); // 固定+1时区arvin20211221
// date_default_timezone_set("Asia/Taipei");	// 固定+8时区arvin20211227
ini_set('date.timezone', 'Etc/GMT-0'); // 固定时区arvin20211227
...
class excel_import extends basic
{
	...
	public $PDO_arr = array(
		PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8',	//設定編碼 連接到 MySQL 服務器時執行的命令。重新連接時會自動重新執行。
		PDO::ATTR_TIMEOUT => 10,	//指定超时的秒数。并非所有驱动都支持此选项，这意味着驱动和驱动之间可能会有差异。比如，SQLite等待的时间达到此值后就放弃获取可写锁，但其他驱动可能会将此值解释为一个连接或读取超时的间隔。 需要 int 类型。
	...
	public function __construct()
	{
		...
		ini_set("max_execution_time", 0);
		$this->error_files = [
			"member_repeat"			=> "import_log/" . $this->prefix . "_member_same_重複帳號略過.log",			//重複帳號略過
			"member_short"			=> "import_log/" . $this->prefix . "_member_short_帳號過短略過.log",			//帳號過短略過
			"member_long"			=> "import_log/" . $this->prefix . "_member_long_帳號過長略過.log",			//帳號過長略過
			"member_error"			=> "import_log/" . $this->prefix . "_member_error_帳號非法字元略過.log",			//帳號非法字元略過
			"member_change"			=> "import_log/" . $this->prefix . "_new_acc_帳號變更清單.log",				//帳號變更清單
			"member_change_error"	=> "import_log/" . $this->prefix . "_member_change_error_帳號變更遞迴超過十次錯誤直接略過.log",	//帳號變更遞迴超過十次錯誤直接略過
			"member_no_name"		=> "import_log/" . $this->prefix . "_no_name_沒有姓名.log",	//沒有姓名
			"member_error_name"		=> "import_log/" . $this->prefix . "_error_name_姓名有非法字元.log",	//姓名有非法字元
			"member_error_no_name"		=> "import_log/" . $this->prefix . "_error_no_name_姓名有非法字元变更后没有姓名.log",	//姓名有非法字元变更后没有姓名
			"status_close_frozen"		=> "import_log/" . $this->prefix . "_status_close_frozen_关闭和冻结会员.log",	//关闭和冻结会员
			"banknum_repeat"		=> "import_log/" . $this->prefix . "_banknum_repeat_銀行卡重複.log",	//銀行卡重複
			"all_bank"					=> "import_log/" . $this->prefix . "_all_bank_全部导入的银行名称.log", //全部导入的银行名称
			"all_banklevel"					=> "import_log/" . $this->prefix . "_all_banklevel_全部导入的會員組.log" //全部导入的會員組
		];
		if ($this->isupdate == 0) {
	...
	private function loadexcel($type)
	{
		...
		if (!file_exists($file)) {
			dump(color($file . "不存在" . PHP_EOL, 'Light Red'));
			die;
		}
		...
		for ($i = $excel_setting['start_row']; $i <= $highestRow; $i++) {
			...
			foreach ($excel_setting['cells'] as $k => $v) {
				...
				// print_r(PHPExcel_Shared_Date::isDateTime($cell).PHP_EOL);
				if (PHPExcel_Shared_Date::isDateTime($cell)) {
					// print_r($cell->getValue().PHP_EOL);
					
					// $row_arr[$k] = date("Y-m-d H:i:s", PHPExcel_Shared_Date::ExcelToPHP(intval($cell->getValue())));	//arvin20211227
					$row_arr[$k] = date("Y-m-d H:i:s", PHPExcel_Shared_Date::ExcelToPHP($cell->getValue()));
					// print_r($k . ':' . $row_arr[$k].PHP_EOL);
				}
			}
			$arrtemp[] = $row_arr;
		}
	...
	private function importmember()
	{
		...
		if (count($insert_data) > 0) {
			foreach ($insert_data as $row) {
				// print_r($row['reg_time'] . PHP_EOL);	//arvin20211221
				//刪除最後一個字元
				// $row['account'] = substr($row['account'], 0, -1);
				//轉小寫
				...
				if ($this->isupdate == 1 and isset($this->account_update_arr[$row['account']])) {
					...
					//更新備註 arvin20211223
					if ($row['remark'] == null || $row['remark'] == '') continue;
					$accountid = $this->all_member[trim($this->account_update_arr[$row['account']])];	//用trim刪除換行字元
					$cl_data = [];
					$cl_data['remark']	= $row['remark'];
					// -, 資料表, 更新資料, 更新id, -
					$this->db_update($db_con['agency'], 'a_account_data', $cl_data, 'accountid = ' . $accountid, $db_con);
					continue;
				}
		...
		if ($times > 10) { //遞迴了十次直接掉過
			...
			if ($this->samechange == 1) {
				...
				if (strlen($str) >= (15 - strlen($this->prefix))) {
					$new_str = substr($str, 0, -$times);
					for ($i = 0; $i < $times; $i++) {
						$new_str = $new_str . 'x';
					}
					// print_r($new_str . PHP_EOL);
				} else {
					$new_str = $str . 'x';
				}
			...
			if ($this->longchange == 1) {
				...
				// $times++;	// 過長不計算次數刪到夠為止 配合從最後往前補x arvin20211221
				$str = $this->check_accountcode($new_str, $old, $times, $name);
			} else {
				if ($times > 1) {
					$save_str = $old . "\t" . $str;
			...
			file_put_contents($this->error_files['member_change'], $old . "\t" . $this->prefix . $str . PHP_EOL, FILE_APPEND);	//匯入帳號
		}
		return $str;
	}
	...
	public function db_con($assign = null)
	{
		...
		foreach ($db['connection'] as $k => $v) {
			if (is_null($assign) || (isset($assign) && $assign == $k)) {
				...
				$db_con[$k]->query('set wait_timeout=28800;');	//服务器在关闭非交互式连接之前等待活动的秒数。在线程启动时，会话wait_timeout 值从全局wait_timeout 值或全局interactive_timeout 值初始化，这取决于客户端的类型（由mysql_real_connect() 的CLIENT_INTERACTIVE 连接选项定义）。 另见interactive_timeout。
				$db_con[$k]->query('set interactive_timeout=28800;');	//服务器在关闭交互式连接之前等待活动的秒数。 交互式客户端被定义为使用 mysql_real_connect() 的 CLIENT_INTERACTIVE 选项的客户端。 另见wait_timeout。
			}
		}
		return $db_con;
	}
